import { Link, useLocation } from "react-router-dom";
import { cn } from "@/lib/utils";
import { 
  LayoutDashboard, 
  BarChart3,
  Users,
  Bug,
  Settings
} from "lucide-react";
import { useUser } from "@/contexts/UserContext";
import { UnifiedAvatar } from "@/components/ui/unified-avatar";

export function AdminNavigation() {
  const location = useLocation();
  const { currentUser } = useUser();
  
  const navItems = [
    {
      href: "/admin",
      label: "Overview",
      icon: LayoutDashboard,
      iconColor: "text-primary",
      isActive: location.pathname === "/admin" && !location.search.includes("tab=")
    },
    {
      href: "/admin?tab=metrics",
      label: "Metrics",
      icon: BarChart3,
      iconColor: "text-blue-500",
      isActive: location.search.includes("tab=metrics")
    },
    {
      href: "/admin?tab=users",
      label: "Users",
      icon: Users,
      iconColor: "text-green-500",
      isActive: location.search.includes("tab=users")
    },
    {
      href: "/admin/debugger",
      label: "Debugger",
      icon: Bug,
      iconColor: "text-orange-500",
      isActive: location.pathname === "/admin/debugger"
    },
    {
      href: "/admin?tab=settings",
      label: "Settings",
      icon: Settings,
      iconColor: "text-purple-500",
      isActive: location.search.includes("tab=settings")
    },
  ];

  return (
    <nav className="flex items-center gap-2 p-2 backdrop-blur-md bg-white/5 border border-white/10 rounded-2xl shadow-lg">
      {/* Avatar Link */}
      <Link
        to="/organizer"
        className="flex items-center justify-center flex-shrink-0"
      >
        <UnifiedAvatar
          userId={currentUser?.id}
          size={40}
          className="sm:w-10 sm:h-10 md:w-10 md:h-10"
        />
      </Link>
      
      {/* Navigation Items */}
      <div className="flex-1 grid grid-cols-3 md:grid-cols-5 gap-2">
        {navItems.map((item) => {
          const Icon = item.icon;
          return (
            <Link
              key={item.href}
              to={item.href}
              className={cn(
                "flex items-center justify-center md:justify-start gap-2 px-3 py-2.5 text-sm font-medium rounded-xl transition-all duration-200",
                "hover:scale-105 hover:shadow-primary/20",
                item.isActive
                  ? "bg-primary/20 border border-primary/30 text-foreground shadow-lg"
                  : "bg-white/5 border border-white/5 text-muted-foreground hover:bg-white/10 hover:border-white/15"
              )}
            >
              <Icon className={cn("h-4 w-4", item.iconColor)} />
              <span className="hidden md:inline truncate">{item.label}</span>
            </Link>
          );
        })}
      </div>
    </nav>
  );
}

