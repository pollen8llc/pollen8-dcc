...
export const FilterDialog: React.FC<FilterDialogProps> = ({
  ...
  // Fetch all unique tags or locations from communities
  useEffect(() => {
    const fetchItems = async () => {
      try {
        if (type === 'tags') {
          ...
          data?.forEach(community => {
            if (community.location && community.location.trim()) {
              uniqueLocations.add(community.location.trim());
            }
          });
          
          setAllItems(Array.from(uniqueLocations).sort());
        }
      } catch (error) {
