...
// Enhanced service request retrieval for service providers
export const getServiceProviderRequests = async (serviceProviderId: string) => {
  const { data, error } = await supabase
    .from('modul8_service_requests')
    .select(`
      *,
...
// Get service requests available for a service provider (by domain specialization)
export const getAvailableServiceRequests = async (userId: string) => {
  // First get the service provider's domain specializations
  ...
  if (providerError) throw providerError;
  if (!provider || !provider.domain_specializations) return [];

  // Get service requests in those domains that are still pending
  const { data, error } = await supabase
    .from('modul8_service_requests')
    .select(`
      *,
