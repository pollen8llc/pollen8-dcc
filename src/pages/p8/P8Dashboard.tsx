        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="lg:col-span-2 p-6 bg-background/40 backdrop-blur-xl border-primary/20 space-y-4">
            ...
            <div className="space-y-3">
              {demographics.map(demo => <div key={demo.label} className="space-y-2">
                  <div className="flex items-center justify-between text-sm">
                    <span>{demo.label}</span>
                    <span className="font-medium">{demo.value}%</span>
                  </div>
                  <div className="h-2 bg-background/60 rounded-full overflow-hidden">
                    <div className={`h-full ${demo.color} transition-all duration-500`} style={{
                  width: `${demo.value}%`
                }} />
                  </div>
                </div>)}
          ...
          <Card className="p-6 bg-background/40 backdrop-blur-xl border-primary/20 space-y-4">
            ...
            <div className="space-y-3">
              {milestones.map(milestone => <div key={milestone.id} className={`flex items-start space-x-3 p-3 rounded-lg transition-all ${milestone.completed ? "bg-primary/10" : "bg-background/60"}`}>
          ...
          <Card className="lg:col-span-2 p-6 bg-background/40 backdrop-blur-xl border-primary/20 space-y-4">
            ...
            <div className="space-y-3">
              {recentActivity.map((activity, index) => <div key={index} className="flex items-center justify-between p-3 bg-background/60 rounded-lg hover:bg-background/80 transition-all">
          ...
          <Card className="p-6 bg-background/40 backdrop-blur-xl border-primary/20 space-y-4">
            ...
            <div className="flex items-center justify-center h-32">
              <div className="relative">
                ...
                <div className="absolute inset-0 w-24 h-24 rounded-full border-8 border-primary border-t-transparent animate-spin" style={{
