...
const Dashboard = () => {
  ...
  // Fetch outreach activities - pass "all" as the default tab
  const { data: outreachActivities = [], isLoading } = useQuery({
    queryKey: ['outreach-activities'],
  ...
  const filteredActivities = activeTab === 'all'
    ? outreachActivities
    : activeTab === 'pending'
      ? outreachActivities.filter(n => n.status === 'pending')
      : outreachActivities.filter(n => n.status === 'completed');

  const pendingCount = outreachActivities.filter(n => n.status === 'pending').length;
  const completedCount = outreachActivities.filter(n => n.status === 'completed').length;
